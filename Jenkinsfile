            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t 146.148.108.150:8083/springapp:${VERSION} .
                                docker login -u admin -p $docker_password 34.125.214.226:8083 
                                docker push  146.148.108.150:8083/springapp:${VERSION}
                                docker rmi 146.148.108.150:8083/springapp:${VERSION}
                            '''
                    }
                }
            }
